...
void checkExpirationAndNotify() async {
  ...
  print("✅ [BACKGROUND] 사용예정일 체크 작업을 시작합니다.");
  ...
  try {
    ...
    for (var product in allGoods) {
      if (product.useDate != null) {
        ...
        if (difference == 0) {
          expiringTodayCount++;
        } else if (difference > 0 && difference <= daysBefore) {
          expiringSoonCount++;
        }
      }
    }
    ...
    if (expiringTodayCount > 0)
      notificationBody += '오늘까지인 음식이 ${expiringTodayCount}개 있습니다. ';
    if (expiringSoonCount > 0)
      notificationBody +=
          '${daysBefore}일 내로 만료되는 음식이 ${expiringSoonCount}개 있습니다.';

    if (notificationBody.isNotEmpty) {
      await LocalNotification.initialize();
      await LocalNotification.display(0, '사용예정일 알림', notificationBody.trim());
