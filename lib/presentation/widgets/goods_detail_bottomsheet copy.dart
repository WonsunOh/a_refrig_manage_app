...
import '../screens/refrig_input_.dart';
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            Text(
              '구매일: ${product.inputDate?.toLocal().toString().split(' ')[0] ?? '-'}',
            ),
            const SizedBox(height: 8),
            Text(
              '사용예정일: ${product.useDate?.toLocal().toString().split(' ')[0] ?? '-'}',
            ),
            const SizedBox(height: 8),
            SingleChildScrollView(child: Text('메모: ${product.memo ?? ''}')),
                    ...
                    final bool? shouldDelete = await Get.dialog<bool>(
                      AlertDialog(
                        ...
                        actions: [
                          TextButton(
                            onPressed: () =>
                                Get.back(result: false), // '취소'시 false 반환
                            child: const Text('취소'),
                          ),
                          TextButton(
                            onPressed: () =>
                                Get.back(result: true), // '삭제'시 true 반환
                            child: const Text(
                              '삭제',
